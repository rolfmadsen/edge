...
## 🎯 Formål
1. Gøre "FDA Modelregler v2.1" i footeren til et klikbart link / knap, som åbner den officielle vejledning (`https://arkitektur.digst.dk/node/770`) i brugerens standardbrowser.
...
## 📋 Acceptance Criteria
- [x] Klik på `FDA Modelregler v2.1` i footeren åbner `https://arkitektur.digst.dk/node/770` via systembrowser.
- [x] `SaveStatus::Saved` udvides eller suppleres med et tidsstempel for gemningstidspunktet.
- [x] Footer-teksten formateres som `💾 Sidst gemt kl. HH:MM:SS • <filnavn>` når en model er gemt.
- [x] Ved ikke-gemte ændringer vises fortsat tydelig indikation (`⚠️ Ikke gemte ændringer` eller `⚠️ Nyt projekt`).
...
- 2026-09-20: Implementeret tidsstempel i `SaveStatus::Saved`, interaktiv modelregler-knap og tooltips. Verificeret og markeret DONE.
