...
## 📋 Acceptance Criteria
- [ ] `ModelProject` i `src/features/model/mod.rs` indeholder `concept_graph: ConceptGraph`, som serialiseres og deserialiseres deterministisk via `ProjectStorage`.
- [ ] `ConceptGraph` i `src/features/concept_model/mod.rs` udvides med metoder til synkronisering med `project.concepts()`, kaskadesletning af relationer når begreber fjernes, samt oprettelse og fjernelse af relationer (Generalisering & Association).
- [ ] `src/ui/graph_canvas.rs` implementerer `iced::widget::canvas::Program` og renderer:
...
- [ ] Værktøjslinje under Fane 3 stiller knapper til rådighed for:
...
- [ ] Ændringer i grafens layout (drag drop) og relationer udløser automatisk autosave til projektfilen.
- [ ] 100% test pass rate på unit-, graf- og accepttests samt clippy med 0 advarsler.
